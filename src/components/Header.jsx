      ...
      <div className="flex justify-center gap-1.5 flex-wrap">
        {prizes.length > 0
          ? prizes.map((prize, i) => (
              <div
                key={prize.numero}
                className="w-16 rounded-lg border-2 border-white/50 shadow-md flex flex-col items-center overflow-hidden bg-white/90"
              >
                <img
          ...
          : [1, 2, 3].map((n) => (
              <div
                key={n}
                className="w-16 h-20 rounded-lg border-2 border-white/50 shadow-md bg-indigo-800 animate-pulse"
              />
            ))}
      </div>
    </header>
